...
// Shared constant so a wrong password and an unknown email return the same response.
...
// Appends -2, -3, etc. until the slug is unique, since it's used in public booking URLs.
...
// Creates the user, business, and a default booking form together. No token is issued until an admin approves the business.
router.post('/register', async (req, res) => {
  ...
  // Quick check for the common case. The transaction below still catches two signups with the same email at the same time.
  ...
  let business
  try {
    // All four inserts happen together, so a failure partway through can't leave a user with no matching business.
    ...
    // Catches the same race as above. This adapter reports the failing table on err.meta.modelName, not err.meta.target.
  ...
  // Each email is sent independently, so one failure can't block the response or the other email.
...
// Blocks pending, rejected, suspended, or deactivated accounts, then issues a role-scoped token.
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Checked before the role branch since it applies to both admins and owners. An admin can deactivate one login without suspending the whole business.
    if (!user.isActive) {
    ...
    let business
    if (user.role === 'owner') {
      // Registration always creates one business per owner, so a missing one means something broke earlier. Treat it as a server error, not a user error.
    ...
    // Owner tokens include businessId so later requests can check ownership without a database lookup. Admin tokens don't need one.
    const payload =
      user.role === 'owner'
...
// Always returns the same response and never waits on the email, so timing can't reveal whether the account exists.
...
// Validates the reset token, then updates the password and clears the token in the same call so the link can't be reused.
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    // No match, missing expiry, or expired all return the same generic message, so a guess can't reveal which case it was.
    ...
    // Password change and token clearing happen in one update, so the old token can never work again after a reset.
    await prisma.user.update({
      where: { id: user.id },
